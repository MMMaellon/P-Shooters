...
// Box light isn't finished, so it's disabled.

Shader "Silent/Volumetric Box"
{
    Properties
    {
        ...
        [Space]
        _MinimumSize("Light Volumetric Radius", Range(0.01, 1)) = 0.01
...
CBUFFER_END
        ...
        Pass
        {
            ...
            fixed4 pixel_shader(v2f ps ) : SV_TARGET
            {
                ...
                // #if !defined(BLOOM)
                // finalLight = renderFakeLightBox(baseWorldPos, viewDirection, depthPosition, _Scale
                //     , _MinimumSize) * _LightPow;
                // #endif
                #if !defined(FINALPASS)
                finalSphere = renderVolumetricBox(baseWorldPos, viewDirection, depthPosition, _Scale
