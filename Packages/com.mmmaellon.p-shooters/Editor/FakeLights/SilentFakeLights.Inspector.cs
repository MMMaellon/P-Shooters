...
namespace SilentFakeLights.Unity
{
	public class Inspector : ShaderGUI
	{
		public class FakeLightsBoot : AssetPostprocessor {
			private static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets,
			string[] movedFromAssetPaths) {
			var isUpdated = importedAssets.Any(path => path.StartsWith("Assets/")) &&
							importedAssets.Any(path => path.Contains("FakeLights_InspectorData"));
		...
		public static void LoadInspectorData()
		{
			...
			//if (styles.Count == 0)
			{
					string[] guids = AssetDatabase.FindAssets("t:TextAsset FakeLights_InspectorData." + Application.systemLanguage);
					if (guids.Length == 0)
					{
						guids = AssetDatabase.FindAssets("t:TextAsset FakeLights_InspectorData.English");
					}
        ...
        protected void DrawShaderPropertySameLine(string i) {
        	int ExHeader_HEIGHT = 22; // Arktoon default
		...
		public static class DefaultStyles
		{
			...
			public static GUIStyle sectionExHeader;
			public static GUIStyle sectionExHeaderBox;
            static DefaultStyles()
            {
				scmStyle = new GUIStyle("DropDownButton");
				sectionExHeader = new GUIStyle(EditorStyles.miniBoldLabel);
				sectionExHeader.padding.left = 24;
				sectionExHeader.padding.right = -24;
				sectionExHeaderBox = new GUIStyle( GUI.skin.box );
				sectionExHeaderBox.alignment = TextAnchor.MiddleLeft;
				sectionExHeaderBox.padding.left = 5;
				sectionExHeaderBox.padding.right = -5;
				sectionExHeaderBox.padding.top = 0;
				sectionExHeaderBox.padding.bottom = 0;
			}
		}

		protected Rect DrawSectionExHeaderArea(GUIContent content)
		{
            Rect r = EditorGUILayout.GetControlRect(true,0,EditorStyles.layerMaskField);
				r.x -= 2.0f;
				r.y += 2.0f;
				r.height = 18.0f;
				r.width -= 0.0f;
			GUI.Box(r, EditorGUIUtility.IconContent("d_FilterByType"), DefaultStyles.sectionExHeaderBox);
			EditorGUILayout.LabelField(content, DefaultStyles.sectionExHeader);
			return r;
		}

		public class HeaderExDecorator : MaterialPropertyDrawer
    	{
	        private readonly string ExHeader;

	        public HeaderExDecorator(string ExHeader)
	        {
	            this.ExHeader = ExHeader;
	        }

	        // so that we can accept ExHeader(1) and display that as text
	        public HeaderExDecorator(float ExHeaderAsNumber)
	        {
	            this.ExHeader = ExHeaderAsNumber.ToString();
	        }
	        ...
	        {/*
	            ...
	            GUI.Label(position, ExHeader, EditorStyles.boldLabel);
*/
            Rect r = position;
				r.x -= 2.0f;
				r.y += 2.0f;
			...
			GUI.Box(r, EditorGUIUtility.IconContent("d_FilterByType"), DefaultStyles.sectionExHeaderBox);
			position.y += 2;
			GUI.Label(position, ExHeader, DefaultStyles.sectionExHeader);
	        }
    	}
...
// https://github.com/Unity-Technologies/UnityCsReference/blob/61f92bd79ae862c4465d35270f9d1d57befd1761/Editor/Mono/Inspector/MaterialEditor.cs#L1468
		...
		{ 
            ...
            //DrawSectionExHeaderArea(Content("s_mainOptions"));
